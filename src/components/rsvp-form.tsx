...
export default function RsvpForm() {
  ...
  const handleSurnameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        alert('No guests found with that surname. Please check your spelling or contact the couple.')
      } else {
        setGuests(data)
  ...
  if (stage === 'surname') {
    return (
      <form onSubmit={handleSurnameSubmit} className="max-w-md mx-auto space-y-4">
        <div>
    ...
    return (
      <form onSubmit={handleRsvpSubmit} className="max-w-md mx-auto space-y-4">
        <div className="bg-gray-50 p-4 rounded-md mb-4">
          <h2 className="text-xl font-semibold">RSVP for {selectedGuest?.first_name} {selectedGuest?.last_name}</h2>
          {selectedGuest && selectedGuest.party_size > 1 && (
            <p className="text-sm text-gray-600">Party of {selectedGuest.party_size}</p>
          )}
        </div>
        ...
        {rsvpData.attending && (
          <div>
            <label htmlFor="dietary_requirements" className="block text-sm font-medium mb-1">
